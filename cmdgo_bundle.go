...
func init() {
	...
	if cmdgo_buildContext.GOOS == "darwin" {
		switch cmdgo_buildContext.GOARCH {
		case "arm", "arm64":
...
func cmdgo_hasString(strings []string, s string) bool {
...
func (cmdgo_gcToolchain) asm(b *cmdgo_builder, p *cmdgo_Package, obj, ofile, sfile string) error {
	...
	args := []interface{}{cmdgo_buildToolExec, cmdgo_tool("asm"), "-o", ofile, "-trimpath", b.work, "-I", obj, "-I", inc, "-D", "GOOS_" + cmdgo_goos, "-D", "GOARCH_" + cmdgo_goarch, cmdgo_buildAsmflags, sfile}
...
func (tools cmdgo_gccgoToolchain) ld(b *cmdgo_builder, root *cmdgo_action, out string, allactions []*cmdgo_action, mainpkg string, ofiles []string) error {
	...
	apackagesSeen := make(map[*cmdgo_Package]bool)
	afiles := []string{}
	shlibs := []string{}
	xfiles := []string{}
	...
	default:
		cmdgo_fatalf("-buildmode=%s not supported for gccgo", cmdgo_ldBuildmode)
	}
	...
	if err := b.run(".", root.p.ImportPath, nil, tools.linker(), "-o", out, ofiles, ldflags, cmdgo_buildGccgoflags); err != nil {
		return err
	}
...
func (tools cmdgo_gccgoToolchain) ldShared(b *cmdgo_builder, toplevelactions []*cmdgo_action, out string, allactions []*cmdgo_action) error {
	...
	return b.run(".", out, nil, tools.linker(), args, cmdgo_buildGccgoflags)
}
...
func (b *cmdgo_builder) cflags(p *cmdgo_Package, def bool) (cppflags, cflags, cxxflags, ldflags []string) {
	var defaults string
	if def {
		defaults = "-g -O2"
	}
...
func (b *cmdgo_builder) cgo(p *cmdgo_Package, cgoExe, obj string, pcCFLAGS, pcLDFLAGS, cgofiles, gccfiles, gxxfiles, mfiles []string) (outGo, outObj []string, err error) {
...
func (g *cmdgo_Generator) identLength(word string) int {
	for i, r := range word {
...
func cmdgo_downloadPackage(p *cmdgo_Package) error {
	...
	if p.build.SrcRoot != "" {
		...
		vcs, rootPath, err = cmdgo_vcsForDir(p)
		if err != nil {
			return err
		}
		repo = "<local>"
...
var cmdgo_documentationTemplate = `// Copyright 2011 The Go Authors.  All rights reserved.
...
func cmdgo_help(args []string) {
	...
	if arg == "documentation" {
		...
		cmdgo_tmpl(os.Stdout, cmdgo_documentationTemplate, append([]*cmdgo_Command{usage}, cmdgo_commands...))
		return
	}
...
func cmdgo_runOut(dir string, cmdargs ...interface{}) []byte {
	cmdline := cmdgo_stringList(cmdargs...)
	cmd := exec.Command(cmdline[0], cmdline[1:]...)
	cmd.Dir = dir
	out, err := cmd.CombinedOutput()
...
var cmdgo_go15VendorExperiment = os.Getenv("GO15VENDOREXPERIMENT") != "0"
...
func cmdgo_loadImport(path, srcDir string, parent *cmdgo_Package, stk *cmdgo_importStack, importPos []token.Position, mode int) *cmdgo_Package {
	...
	if !cmdgo_go15VendorExperiment || mode&cmdgo_useVendor == 0 || path != origPath {
	...
	if err == nil && !isLocal && bp.ImportComment != "" && bp.ImportComment != path &&
		(!cmdgo_go15VendorExperiment || (!strings.Contains(path, "/vendor/") && !strings.HasPrefix(path, "vendor/"))) {
...
func cmdgo_vendoredImportPath(parent *cmdgo_Package, path string) (found string) {
	if parent == nil || parent.Root == "" || !cmdgo_go15VendorExperiment {
		return path
	}
...
var cmdgo_goTools = map[string]cmdgo_targetDir{
	...
	"golang.org/x/tools/cmd/godoc":         cmdgo_toBin,
...
func (p *cmdgo_Package) load(stk *cmdgo_importStack, bp *build.Package, err error) *cmdgo_Package {
	...
	if useBindir {
		...
		if p.build.BinDir != cmdgo_gobin && cmdgo_goTools[p.ImportPath] == cmdgo_toBin {

			p.target = filepath.Join(cmdgo_gorootBin, elem)
...
func cmdgo_isStale(p *cmdgo_Package) bool {
	if p.Standard && (p.ImportPath == "unsafe" || cmdgo_buildContext.Compiler == "gccgo") {
	...
	srcs := cmdgo_stringList(p.GoFiles, p.CFiles, p.CXXFiles, p.MFiles, p.HFiles, p.SFiles, p.CgoFiles, p.SysoFiles, p.SwigFiles, p.SwigCXXFiles)
...
func cmdgo_isTestMain(fn *ast.FuncDecl) bool {
...
func (t *cmdgo_testFuncs) load(filename, pkg string, doImport, seen *bool) error {
	...
	for _, d := range f.Decls {
		...
		name := n.Name.String()
		switch {
		case cmdgo_isTestMain(n):
			if t.TestMain != nil {
				return errors.New("multiple definitions of TestMain")
			}
			t.TestMain = &cmdgo_testFunc{pkg, name, ""}
			*doImport, *seen = true, true
		case cmdgo_isTest(name, "Test"):
			t.Tests = append(t.Tests, cmdgo_testFunc{pkg, name, ""})
			*doImport, *seen = true, true
		case cmdgo_isTest(name, "Benchmark"):
			t.Benchmarks = append(t.Benchmarks, cmdgo_testFunc{pkg, name, ""})
			*doImport, *seen = true, true
		}
	}
	ex := doc.Examples(f)
	...
	for _, e := range ex {
		*doImport = true
		...
		t.Examples = append(t.Examples, cmdgo_testFunc{pkg, "Example" + e.Name, e.Output})
		*seen = true
	}
	return nil
}
...
func (v *cmdgo_vcsCmd) create(dir, repo string) error {
	for _, cmd := range v.createCmd {
		if !cmdgo_go15VendorExperiment && strings.Contains(cmd, "submodule") {
			continue
		}
...
func cmdgo_vcsForDir(p *cmdgo_Package) (vcs *cmdgo_vcsCmd, root string, err error) {
...
var cmdgo_googleCheckout = regexp.MustCompile(`id="checkoutcmd">(hg|git|svn)`)

// googleCodeVCS determines the version control system for
// a code.google.com repository, by scraping the project's
// /source/checkout page.
func cmdgo_googleCodeVCS(match map[string]string) error {
	if err := cmdgo_noVCSSuffix(match); err != nil {
		return err
	}
	data, err := cmdgo_httpGET(cmdgo_expand(match, "https://code.google.com/p/{project}/source/checkout?repo={subrepo}"))
	if err != nil {
		return err
	}

	if m := cmdgo_googleCheckout.FindSubmatch(data); m != nil {
		if vcs := cmdgo_vcsByCmd(string(m[1])); vcs != nil {

			if vcs == cmdgo_vcsSvn {
				if match["subrepo"] != "" {
					return fmt.Errorf("sub-repositories not supported in Google Code Subversion projects")
				}
				match["repo"] = cmdgo_expand(match, "https://{project}.googlecode.com/svn")
			}
			match["vcs"] = vcs.cmd
			return nil
		}
...
func cmdgo_oldGoogleCode(match map[string]string) error {
	return fmt.Errorf("invalid Google Code import path: use %s instead",
		cmdgo_expand(match, "code.google.com/p/{project}{path}"))
}
