...
func main() {
	...
	cmdList.Flags().AddFlag(flagVerbose)
	cmdList.Flags().AddFlag(flagQuiet)
	cmdList.Flags().AddFlag(flagMinify)
	cmdList.Flags().AddFlag(flagColor)
	cmdList.Flags().AddFlag(flagTags)
	cmdList.Run = func(_ *cobra.Command, args []string) {
		options.BuildTags = strings.Fields(*tags)

		build.Default.GOARCH = "js"
		cmdgo_buildContext = build.Default

		os.Args = []string{"gopherjs", "list", "-compiler=gopherjs"}
		if options.Verbose {
			os.Args = append(os.Args, "-v")
		}
		if *tags != "" {
	...
	// TODO: Remove, this is for debugging only.
	// Previous code that shells out, for reference.
	/*cmdListOld := &cobra.Command{
		Use:   "list-old [packages]",
		Short: "list packages",
	}
	cmdListOld.Flags().AddFlag(flagVerbose)
	cmdListOld.Flags().AddFlag(flagQuiet)
	cmdListOld.Flags().AddFlag(flagMinify)
	cmdListOld.Flags().AddFlag(flagColor)
	cmdListOld.Flags().AddFlag(flagTags)
	cmdListOld.Run = func(_ *cobra.Command, args []string) {
		options.BuildTags = strings.Fields(*tags)

		exitCode := handleError(func() error {
			// TODO: Don't shell out, use cmdgo_bundle.go.
			cmd := exec.Command("go", append([]string{"list", "-compiler=gopherjs"}, args...)...)
	...
	rootCmd.AddCommand(cmdList /*, cmdListOld*/)
	rootCmd.Execute()
}
