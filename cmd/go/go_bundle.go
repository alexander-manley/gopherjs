...
//go:generate bundle -o go_bundle.go -import github.com/gopherjs/go/internal/singleflight=github.com/gopherjs/gopherjs/internal/singleflight github.com/gopherjs/go/cmd/go
...
func (c buildCompiler) Set(value string) error {
	...
	case "gopherjs":
		...
		buildToolchain = gopherjsToolchain{binaryPath: exe}
	default:
...
func (t gopherjsToolchain) compiler() string {
	return t.binaryPath
}

func (t gopherjsToolchain) linker() string {
	return t.binaryPath
}

// gc runs the compiler in a specific directory on a set of files
// and returns the name of the generated output file.
func (gopherjsToolchain) gc(b *builder, p *Package, archive, obj string, asmhdr bool, importArgs []string, gofiles []string) (ofile string, out []byte, err error) {
	if archive != "" {
		ofile = archive
	} else {
		out := "_go_.o"
	...
	if p.build.IsCommand() && (!p.forceLibrary || p.ImportPath == "testmain") {
		...
		pkg, err := Import(p.ImportPath, 0, "", nil)
		if err != nil {
			return "", nil, err
		}
		...
		archive, err := BuildPackage(pkg, s, options, importFromDisk)
		if err != nil {
...
// ld runs the linker to create an executable starting at mainpkg.
func (g gopherjsToolchain) ld(b *builder, root *action, out string, allactions []*action, mainpkg string, ofiles []string) error {
	if root.p.Name != "main" {
...
// ldShared runs the linker to create a shared library containing the pkgs built by toplevelactions
func (gopherjsToolchain) ldShared(b *builder, toplevelactions []*action, out string, allactions []*action) error {
	return fmt.Errorf("GopherJS has not implemented support for shared libraries")
}

func BuildPackage(pkg *gopherjsbuild.PackageData, s *gopherjsbuild.Session, options *gopherjsbuild.Options, importLoader func(path string) (*compiler.Archive, error)) (*compiler.Archive, error) {
...
// *PackageData.
func Import(path string, mode build.ImportMode, installSuffix string, buildTags []string) (*gopherjsbuild.PackageData, error) {
...
func importWithSrcDir(path string, srcDir string, mode build.ImportMode, installSuffix string, buildTags []string) (*gopherjsbuild.PackageData, error) {
...
// pkgpath builds an appropriate path for a temporary package file.
func (gopherjsToolchain) pkgpath(basedir string, p *Package) string {
...
// cc runs the toolchain's C compiler in a directory on a C file
// to produce an output file.
func (gopherjsToolchain) cc(b *builder, p *Package, objdir, ofile, cfile string) error {
...
func (b *builder) test(p *Package) (buildAction, runAction, printAction *action, err error) {
	...
	if testC {
		printAction = &action{p: p, deps: []*action{runAction}}
	} else {
